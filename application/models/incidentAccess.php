...
class incidentAccess extends CI_Model {
	...
	function addNewIncident($classification, $desc, $start, $end, $street, $brgy, $latitude, $longitude, $start, $end, $has_line, $start_lat, $start_long, $end_lat, $end_long){		
		...
		if($start_lat!=NULL ||  $start_long!=NULL ||  $end_lat!=NULL || $end_long!=NULL){
			$status2 = $this->db->query("INSERT into map_coordinates (map_id, latitude, longitude, street, barangay, inc_id, line_start_lat, line_start_long, line_end_lat, line_end_long) values ('$id', '$latitude', '$longitude', '$street', '$brgy', '$id',  '$start_lat', '$start_long', '$end_lat', '$end_long')");
		}else{
